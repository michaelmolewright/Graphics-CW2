...
SimpleMeshData createSphere( Mat44f preTransform ) {
    ...
    SimpleMeshData dome4 =
        createDome( triangle1, make_rotation_z( -PI / 2.f ) );

    SimpleMeshData dome5 = createDome( triangle1, make_rotation_x( PI / 2.f ) );
    SimpleMeshData dome6 = createDome(
        triangle1, make_rotation_z( PI / 2.f ) * make_rotation_x( PI / 2.f ) );
    SimpleMeshData dome7 = createDome(
        triangle1, make_rotation_z( PI ) * make_rotation_x( PI / 2.f ) );
    SimpleMeshData dome8 = createDome(
        triangle1, make_rotation_z( -PI / 2.f ) * make_rotation_x( PI / 2.f ) );

    SimpleMeshData ret1 =
        concatenate( concatenate( dome1, dome2 ), concatenate( dome3, dome4 ) );
    SimpleMeshData ret2 =
        concatenate( concatenate( dome5, dome6 ), concatenate( dome7, dome8 ) );
...
std::vector<Vec3f> transformPoints( std::vector<Vec3f> input,
                                    Mat44f transform ) {
    for ( auto &p : input ) {
...
std::vector<Vec3f> transformNormals( std::vector<Vec3f> input,
                                     Mat44f transform ) {
    for ( auto &p : input ) {
        Vec4f p4{ p.x, p.y, p.z, 1.f };
        Vec4f t = transform * p4;
...
std::vector<Vec3f> createTriangles( std::vector<Vec3f> pos, tri abc,
                                    int counter ) {
    counter -= 1;
    if ( counter == 0 ) {
        return pos;
...
SimpleMeshData createFloor( Mat44f preTransform ) {
    ...
    SimpleMeshData floor =
        SimpleMeshData{ std::move( floorTris ), std::move( normals ) };
...
SimpleMeshData make_bowl_cylinder( std::size_t aSubdivs, Mat44f aPreTransform,
                                   float width ) {
    ...
    for ( std::size_t i = 0; i < aSubdivs; ++i ) {
        ...
        norms.emplace_back(
            -normalize( middleLeft - Vec3f{ 0.f, prevY, prevZ } ) );
        norms.emplace_back(
            -normalize( middleRight - Vec3f{ width, prevY, prevZ } ) );
...
SimpleMeshData createFinalForm( Mat44f preTransform ) {
    ...
    SimpleMeshData floor2 = createFloor(
        make_rotation_z( PI ) * make_translation( { length, 0.f, 0.f } ) );
    make_scaling( 1.f, 2.f, 2.f ) * make_translation( { -length, 0.f, 0.f } );

    SimpleMeshData cyclinder =
        make_bowl_cylinder( 1000,
                            make_scaling( 1.f, 2.f, 2.f ) *
                                make_translation( { -length, 0.f, 0.f } ),
                            length );

    SimpleMeshData ret = concatenate( floor1, floor2 );
    ret = concatenate( ret, cyclinder );

    SimpleMeshData tile1 = createTile( make_translation( { -7.f, -2.f, 2.f } ) *
                                       make_scaling( 9.f, 4.f, 1.f ) );
    ret = concatenate( ret, tile1 );

    SimpleMeshData tile2 = createTile( make_rotation_y( PI / 2.f ) *
                                       make_translation( { -2.f, -2.f, 2.f } ) *
                                       make_scaling( 2.f, 4.f, 1.f ) );
    ret = concatenate( ret, tile2 );

    SimpleMeshData tile3 = createTile( make_translation( { -7.f, -2.f, 0.f } ) *
                                       make_rotation_y( -PI / 2.f ) *
                                       make_scaling( 2.f, 4.f, 1.f ) );
    ret = concatenate( ret, tile3 );

    SimpleMeshData tile4 = createTile( make_translation( { -7.f, 2.f, 2.f } ) *
                                       make_rotation_x( -PI / 2.f ) *
                                       make_scaling( 9.f, 2.f, 1.f ) );
    ret = concatenate( ret, tile4 );

    SimpleMeshData tile5 = createTile( make_translation( { -7.f, -2.f, 0.f } ) *
                                       make_rotation_x( PI / 2.f ) *
                                       make_scaling( 9.f, 2.f, 1.f ) );
