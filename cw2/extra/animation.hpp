...
Mat44f linearMotion(float distance, float aniLen, float counter, Vec3f direction){
...
Mat44f skateboardAimation(float &counter, float totalTime){
    ...
    if (counter < segment)
        ...
        res = linearMotion(3.f, segment, counter - (segment * 2), {0.f,0.f,1.f});
